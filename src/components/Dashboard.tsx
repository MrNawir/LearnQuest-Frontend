    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">You've learned for 42 minutes today. Keep it up!</p>
        </div>
        <div className="flex items-center gap-2 bg-card p-1 rounded-xl shadow-sm border border-border">
          <button className="px-4 py-2 bg-secondary text-secondary-foreground rounded-lg text-sm font-medium">Daily</button>
          <button className="px-4 py-2 hover:bg-secondary/50 text-muted-foreground rounded-lg text-sm font-medium transition-colors">Weekly</button>
          <button className="px-4 py-2 hover:bg-secondary/50 text-muted-foreground rounded-lg text-sm font-medium transition-colors">Monthly</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Current Streak', value: '12 Days', icon: Flame, color: 'text-orange-500', bg: 'bg-orange-100' },
          { label: 'Total XP', value: '12,450', icon: Star, color: 'text-yellow-600', bg: 'bg-yellow-100' },
          { label: 'Hours Learned', value: '48.5h', icon: Clock, color: 'text-blue-500', bg: 'bg-blue-100' },
          { label: 'Global Rank', value: '#402', icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-100' },
        ].map((stat, i) => (
          <motion.div
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-foreground">Active Learning Paths</h2>
          ...
          <div className="space-y-4">
            {[
              ...
              { 
                ...
                image: 'https://images.unsplash.com/photo-1558655146-d09347e0b7a9?w=500&h=300&fit=crop',
                category: 'Design'
              },
            ].map((path, i) => (
              <motion.div
              ...
              >
                ...
                <div className="flex-1 flex flex-col justify-center gap-3">
                  <div className="flex justify-between items-start">
                    <h3 className="text-lg font-bold text-foreground group-hover:text-accent transition-colors">{path.title}</h3>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm text-foreground font-medium truncate">Up Next: {path.next}</span>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-primary/20 to-accent/10 rounded-2xl border border-primary/20 p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl font-bold text-foreground mb-2">CSS Grid Master</h3>
              <p className="text-sm text-muted-foreground mb-6">Complete the layout challenge with perfect score to win a rare badge.</p>
              ...
              <div className="space-y-3 mb-6">
                 <div className="flex items-center gap-3 text-sm">
                   ...
                   <span className="line-through text-muted-foreground">Theory Check</span>
                 </div>
                 <div className="flex items-center gap-3 text-sm">
                   <div className="w-5 h-5 rounded-full bg-card border-2 border-primary text-primary flex items-center justify-center text-xs font-bold">2</div>
                   <span className="font-medium">Practical Exercise</span>
                 </div>
                 <div className="flex items-center gap-3 text-sm">
                   <div className="w-5 h-5 rounded-full bg-card border-2 border-muted flex items-center justify-center text-xs font-bold text-muted-foreground">3</div>
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3 items-start group cursor-pointer">
                  <div className="w-12 h-12 rounded-lg bg-secondary shrink-0 overflow-hidden">
                    <img src={`https://images.unsplash.com/photo-${i === 1 ? '1605379399642-870262d3d051' : i === 2 ? '1581091226825-a6a2a5aee158' : '1550745165-9bc0b252726f'}?w=100&h=100&fit=crop`} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-foreground group-hover:text-accent transition-colors line-clamp-1">Introduction to Data Science</h4>
